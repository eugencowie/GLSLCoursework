...
vec3 calculateDirectionalLight(DirectionalLight light, vec3 normal, vec3 viewDir)
{
	...
	vec3 diffuse = light.diffuse * max(dot(normal, lightDir), 0) * vec3(texture(material.diffuse, f_TexCoord));
...
vec3 calculatePointLight(PointLight light, vec3 normal, vec3 viewDir)
{
	...
	vec3 diffuse = light.diffuse * max(dot(normal, lightDir), 0) * vec3(texture(material.diffuse, f_TexCoord)) * attenuation;
...
vec3 calculateSpotLight(SpotLight light, vec3 normal, vec3 viewDir)
{
	...
	vec3 diffuse = light.diffuse * max(dot(normal, lightDir), 0) * vec3(texture(material.diffuse, f_TexCoord)) * attenuation * intensity;
...
void main()
{
	...
	vec3 result = ambientLight * vec3(texture(material.diffuse, f_TexCoord));
